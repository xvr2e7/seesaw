...
public class FlowSimulation : MonoBehaviour
{
    ...
    // Agent data - kept as simple arrays for future optimization with NativeArrays/Jobs
    private Vector2[] positions;
    private Vector2[] velocities;
    private Vector2[] desiredDirections;
    
    // Calculated world size based on aspect ratio
    private Vector2 worldSize;
    
    // Public accessors for other systems (FlowVisualizer, etc.)
    ...
    void InitializeAgents()
    {
        ...
        Debug.Log($"[FlowSimulation] Initialized {agentCount} agents in {worldSize.x:F0}x{worldSize.y:F0} world (aspect {targetAspectRatio:F2})");
    }
    void UpdateWanderDirections(float dt)
    {
        for (int i = 0; i < agentCount; i++)
        {
            // Add random angular noise to desired direction
            float noiseAngle = (Random.value - 0.5f) * 2f * wanderStrength * dt;
            
            // Rotate current desired direction by noise
    ...
    void OnDrawGizmos()
    {
        ...
        // Draw agents if running
        if (positions != null && Application.isPlaying)
        {
            Gizmos.color = new Color(0.2f, 0.8f, 0.4f, 0.5f);
            for (int i = 0; i < Mathf.Min(agentCount, 100); i++) // Limit gizmos for performance
            {
